...
class Server(Dataset):
    ...
    def __read_data__(self):
        ...
        border1s = [0, round(len(df_raw)*self.train_ratio*self.val_split) - self.seq_len, round(len(df_raw)*self.train_ratio) - self.seq_len]
        border2s = [round(len(df_raw)*self.train_ratio*self.val_split), round(len(df_raw)*self.train_ratio), len(df_raw)]
        ...
        if self.scale:
            ...
            data = self.scaler.transform(np.reshape(df_data.values, (-1,1)))
        else:
            data = df_data.values
        ...
        if self.timeenc == 0:
            ...
            # time_features：将date中的时间信息按照某年某月某日某小时这样的逻辑分出来（都转变为-0.5~0.5之间的小数），最小的粒度是self.freq，因此data_stamp会从列向量变成多维数组
